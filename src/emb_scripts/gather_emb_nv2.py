...
if __name__ == '__main__':
    ...
    emb_results = {}
    for fn in tqdm(natsorted(args.input_dir.glob(f'doc_set_*.nv_{"inst" if args.inst else "noinst"}.emb')), ncols=100):
        vals = pd.read_pickle(fn)
        emb_results.update(vals)
    ...
    pd.to_pickle(emb_results, f'../../data/intermediates/all_doc_nv2_{"inst" if args.inst else "noinst"}.pkl')